...
// デバッグ機能を使うための初期化関数(screenSizeX,screenSizeY,alpha値)
...
// 一つの内部変数を画面上描画(引数は、文字色,文字,数値用の変数(intもしくは浮動小数点))、複数回呼び出すと自動的に下にずれて表示されます
#define _TRACE_S(COLOR,STR,NUM)				_DebugDispOut::GetInstance().TRACE_S(COLOR,STR,NUM);
// printfと同じ感じで使えます,複数回呼び出すと自動的に下にずれて表示されます
...
public:
	...
	void TRACE_M(int Color, std::string str, ...);
