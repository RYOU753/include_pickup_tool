...
void _DebugDispOut::TRACE_S(int Color, std::string str, int num)
{
	...
	DxLib::DrawBox(mesPosX_, mesPosY_ + drawLineY_, mesPosX_ + length, mesPosY_ + drawLineY_ + 16, 0, true);
...
void _DebugDispOut::TRACE_M(int Color, std::string str, ...)
{
	static TCHAR strBuffer_g[1024];
	va_list ap;
	va_start(ap, str);
#if _DEBUG
	int len = _vsctprintf(str.c_str(), ap);
	if (len >= 1024)
		_ASSERT(0);
#endif
	_vstprintf_s(strBuffer_g, str.c_str(), ap);

	SetScreen();
	auto length = GetDrawFormatStringWidth(strBuffer_g);
	DxLib::DrawBox(mesPosX_, mesPosY_ + drawLineY_, mesPosX_ + length, mesPosY_ + drawLineY_ + 16, 0, true);
	DrawFormatString(mesPosX_, mesPosY_ + drawLineY_, Color, strBuffer_g);
	RevScreen();
	drawLineY_ += 16;
}
...
void _DebugDispOut::DrawFPS(void)
{
	...
	_TRACE_S(0xffffff, "fps", fpsView_)
}
